...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,LPSTR lpszCmdParam, int nCmdShow) {
	...
	if(!RegisterClassEx(&WndClassEx)) {
		MessageBox(0, "无法注册子窗口。请检查系统环境是否正常或寻求技术帮助。", "错误",
		...
		case WM_COMMAND: {
			switch(LOWORD(wParam)) {
				...
				case CM_FILE_NEW: {
					...
					if(!hChild) {
						MessageBox(hwnd, "MDI窗口创建失败。请检查系统环境是否正常或寻求技术帮助。", "错误",MB_ICONEXCLAMATION | MB_OK);
					}
					break;
				}
				case CM_FILE_OPEN: {
					...
					if(!hChild) {
						MessageBox(hwnd, "MDI窗口创建失败。请检查系统环境是否正常或寻求技术帮助。", "错误",
						MB_ICONEXCLAMATION | MB_OK);
					}
					break;
				}
				...
				case CM_FILE_SAVE: {
					...
					if(*szFileName != '[') {
						if(!SaveFile(GetDlgItem(hwnd, IDC_CHILD_EDIT), szFileName)) {
							MessageBox(hwnd, "当前文件为空或保存失败。", "无数据",MB_OK | MB_ICONEXCLAMATION);
							return 0;
						}
					} else {
